...
async def send_test_messages(
    num_messages=10, reliability_type="reliable", delay=0.1, host="127.0.0.1", port=4433
):
    """Send test messages"""
    print(f"\n{'='*70}")
...
def check_message_ordering(received_messages, is_reliable=True):
    ...
    for i, msg in enumerate(filtered_messages):
        ...
        if msg["payload"] != expected_payload:
            return (
                False,
                f"Payload mismatch at position {i}: expected '{expected_payload}', got '{msg['payload']}'",
            )
